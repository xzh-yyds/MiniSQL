...
void BufferManager::refresh_file(FileNode &File){
	memset(File.FileName, 0, MAX_FILE_NAME);   //清空文件名
	//File.preFile=File.nextFile=NULL;
	File.HeadBlock = NULL;			//清空第一个块
	File.pin = false;				//解除锁
	//File.isUsed=false;
}
...
BlockNode* BufferManager::getBlock(FileNode *curFile, BlockNode *curBlock){
	...
	//如果块全部用满了，则使用LRU算法找到一个块
	else{
		int i;
		//在文件池中找到LRU的坑的位置，从上一次修改的位置找起，直到发现LRU为false并且pin为false
	...
	//如果curBlock后面还有block，则将btmp插入到两者之间
	else{
		btmp->preBlock = curBlock;
...
#if MACTESTMODE
	char temp[100] = "C:\Users\xzh\Desktop\MiniSQL\";
...
char* BufferManager::getContent(BlockNode* block){
	return (char*)(sizeof(size_t) + block->address+1);//去掉头信息
}
